...
<div class="sbox">
	<div class="sbox-title"> <?php $this->load->view('notification/toolbar');?> </div>
	
	<div class="sbox-content"> 	

	 <form action='<?php echo site_url('notification/destroy') ?>' class='form-horizontal' id ='notificationForm' method="post" >
        ...
        <tbody>		
			...
			<tr id="form-0" >
				...
				<td style="width:50px;">
					...
					onclick="ajaxInlineSave('#<?php echo $pageModule ;?>','<?php echo site_url('notification/quicksave') ;?>','<?php echo site_url('notification/data?md=') ;?>')">
			...
			?>
                <tr class="editable" id="form-<?php echo $row->id ;?>">
					...
					<td><a href="javascript:void(0)" class="expandable" rel="#row-<?php echo $row->id ;?>" data-url="<?php echo site_url('notification/show/'.$id) ;?>"><i class="fa fa-plus " ></i></a></td>								
					<?php endif;?>						
				 ...
				 <td data-values="action" data-key="<?php echo $row->id ;?>">
					<?php echo AjaxHelpers::buttonAction('notification',$access,$row->id ,$setting , $return);
...
$(document).ready(function(){
	...
	$('#notificationTable .checkall').on('ifChecked',function(){
		$('#notificationTable input[type="checkbox"]').iCheck('check');
	});
	$('#notificationTable .checkall').on('ifUnchecked',function(){
		$('#notificationTable input[type="checkbox"]').iCheck('uncheck');
	});	
	$('.date').datepicker({format:'yyyy-mm-dd',autoClose:true})
	$('#notificationPaginate .pagination li a').click(function() {
		var url = $(this).attr('href');
		if(url != '') {
			reloadData('#notification',url);		
		}			
		return false ;
	});	
