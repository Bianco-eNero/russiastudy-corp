...
<div class="sbox"  >
	...
	<?php $this->load->view('core/notification/toolbar');?>
	</div>
	<div class="sbox-content" style="min-height: 500px;">
			

			<div class="table-responsive">
			<table id="notificationTable" class="table table-striped table-bordered table-hover display" cellspacing="0" width="100%">
		        <thead>
		            <tr>
		            <th>ID</th>	
	...
	$(document).ready(function() {
		...
		var rows_selected = []; 			
	   	var table = $('#notificationTable').DataTable( {
	        ...
	        "ajax": {
	            "url": "<?php echo site_url('core/notification/data');?>",
	            "type": "POST"
        	},
        	"columnDefs": [{ 
        		"targets": [0],
		...
		$('#notificationTable tbody').on('click', 'td.details-control', function () {
	        ...
	        if ( row.child.isShown() ) {
	            ...
	            tr.addClass('shown');
	            $.get('<?php echo site_url("core/notification/show") ;?>/'+id , function(callback){
	            	$('#'+id).html(callback);
	            	$('#'+id).addClass('data');
	            })
	    ...
	    $('#notificationTable').Sdtable({
	    	tableId : '#notification',
	    	table   : table,
	    	action  : '<?php echo site_url("core/notification");?>' 
	    });
