...
<div class="sbox">
	<div class="sbox-title"> <?php $this->load->view('logs/toolbar');?> </div>
	
	<div class="sbox-content"> 	

	 <form action='<?php echo site_url('logs/destroy') ?>' class='form-horizontal' id ='logsForm' method="post" >
        ...
        <tbody>		
			...
			<tr id="form-0" >
				...
				<td style="width:50px;">
					...
					onclick="ajaxInlineSave('#<?php echo $pageModule ;?>','<?php echo site_url('logs/quicksave') ;?>','<?php echo site_url('logs/data?md=') ;?>')">
			...
			<?php foreach ( $rowData as $i => $row ) : 
					 $id = $row->auditID;
			?>
                <tr class="editable" id="form-<?php echo $row->auditID ;?>">
					<td width="50"> <?php echo ($i+1+$page) ?> </td>
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="<?php echo $row->auditID ?>" />  </td>
					<?php if($setting['view-method']=='expand'): ?>
					<td><a href="javascript:void(0)" class="expandable" rel="#row-<?php echo $row->auditID ;?>" data-url="<?php echo site_url('logs/show/'.$id) ;?>"><i class="fa fa-plus " ></i></a></td>								
					<?php endif;?>						
				 <?php foreach ( $tableGrid as $j => $field ) : ?>
					 ...
					 <?php endif; ?>
				 <?php endforeach; ?>
				 <td data-values="action" data-key="<?php echo $row->auditID ;?>">
					<?php echo AjaxHelpers::buttonAction('logs',$access,$row->auditID ,$setting , $return);
						  echo AjaxHelpers::buttonActionInline($row->auditID,'auditID') ;
					?>	
				</td>				 
                </tr>
               <?php if($setting['view-method']=='expand'): ?>
                <tr style="display:none" class="expanded" id="row-<?php echo $row->auditID;?>">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
...
$(document).ready(function(){
	...
	$('#logsTable .checkall').on('ifChecked',function(){
		$('#logsTable input[type="checkbox"]').iCheck('check');
	});
	$('#logsTable .checkall').on('ifUnchecked',function(){
		$('#logsTable input[type="checkbox"]').iCheck('uncheck');
	});	
	$('.date').datepicker({format:'yyyy-mm-dd',autoClose:true})
	$('#logsPaginate .pagination li a').click(function() {
