...
<div class="sbox"  >
	...
	<?php $this->load->view('core/users/toolbar');?>
	</div>
	<div class="sbox-content" style="min-height: 500px;">

<ul class="nav nav-tabs" style="margin-bottom:10px;">
	<li class="active" ><a href="<?php echo site_url('core/users');?>"><i class="icon-user"></i> <?php echo $this->lang->line('core.m_users'); ?> </a></li>
	...
	<li ><a href="<?php echo site_url('core/users/blast');?>"><i class="icon-mail-send"></i> <?php echo $this->lang->line('core.m_blastemail'); ?> </a> </li>
			...
			<table id="usersTable" class="table table-striped table-bordered table-hover display" cellspacing="0" width="100%">
		        <thead>
		            <tr>
		            <th>ID</th>	
	...
	$(document).ready(function() {
		...
		var rows_selected = []; 			
	   	var table = $('#usersTable').DataTable( {
	        ...
	        "ajax": {
	            "url": "<?php echo site_url('core/users/data');?>",
	            "type": "POST"
        	},
        	"columnDefs": [{ 
        		"targets": [0],
		...
		$('#usersTable tbody').on('click', 'td.details-control', function () {
	        ...
	        if ( row.child.isShown() ) {
	            ...
	            tr.addClass('shown');
	            $.get('<?php echo site_url("core/users/show") ;?>/'+id , function(callback){
	            	$('#'+id).html(callback);
	            	$('#'+id).addClass('data');
	            })
	    ...
	    $('#usersTable').Sdtable({
	    	tableId : '#users',
	    	table   : table,
	    	action  : '<?php echo site_url("core/users");?>' 
	    });
