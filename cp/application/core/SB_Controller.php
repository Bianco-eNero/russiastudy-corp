	...
	function validatePost(  $table = '')
	{
		...
		foreach($str as $f){
			...
			if($f['view'] ==1)
			{
				...
				if(!is_null($this->input->post( $field )))
				{
				...
				switch ($f['type']) {
					...
					case 'textarea':
						$content = $this->input->get_post($field)? $this->input->post($field):'';
						$data[$field] =  $content;
...
//BEGIN AJAX ADDITIONAL CODE	
	...
	public function flysearch( $mode = 'ajax')
	{
	...
	function lookup(  $id)
	{
		...
		if(count($args)>=2) 
		{
			...
			$this->load->view('sximo/module/utility/masterdetail',$data);
	...
	function detailviewsave( $model , $detail , $id )
	{
		$submodel = $model.'model';
		$this->load->model($submodel);
		$model = new $submodel();
		...
		$info = $model->makeInfo( $detail['module'] );
		$this->db->delete($detail['table'], array($detail['key'] => $id)); 
		...
		for($i=0; $i<$total;$i++)
		{
			foreach($str as $f){
				...
				if($f['view'] ==1)
				{
					if(isset($_POST['bulk_'.$field][$i]) && $_POST['bulk_'.$field][$i] !='')
					{
						$data[$f['field']] = $_POST['bulk_'.$field][$i];
					}
				}			
			}	
			if($global == 0 )
				$data['entry_by'] = $this->session->userdata('uid');
			$this->db->insert($detail['table'],$data);
		}	
	...
	public function insertNotification( $args)
	{
		...
		$this->db->insert('tb_notification',$args);

	}

	public function insertLogs( $args )
	{
