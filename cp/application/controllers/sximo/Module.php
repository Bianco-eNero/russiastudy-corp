...
class Module extends SB_Controller  {
    ...
    function rebuild( $id = 0)
    {
        ...
        $codes['form_view'] = SiteHelpers::toView($config['grid']);
            ...
            $setting = array(
            'gridtype'        => (isset($config['setting']) ? $config['setting']['gridtype'] : 'ajax'  ),
            'orderby'        => (isset($config['setting']) ? $config['setting']['orderby'] : $row->module_db_key  ),
            'ordertype'        => (isset($config['setting']) ? $config['setting']['ordertype'] : 'asc'  ),
            'perpage'        => (isset($config['setting']) ? $config['setting']['perpage'] : '10'  ),
        );  
         ...
         // BLANK TEMPLATE 
        if($setting['gridtype'] =='generic')
        {

            $controller = file_get_contents(  'application/views/sximo/module/template/generic/controller.tpl' );
            $grid       = file_get_contents(  'application/views/sximo/module/template/generic/index.tpl' ); 
            $model      = file_get_contents(  'application/views/sximo/module/template/generic/model.tpl' );    
            $view       = file_get_contents(  'application/views/sximo/module/template/generic/view.tpl' ); 
            $form       = file_get_contents(  'application/views/sximo/module/template/generic/form.tpl' ); 
            ...
            file_put_contents(  "application/views/{$class}/view.php" , $build_form) ;    
            file_put_contents(  "application/views/{$class}/form.php" , $build_view) ;    
             

        }   

        if($setting['gridtype']  =='report')
        {
            $controller = file_get_contents(  'application/views/sximo/module/template/report/controller.tpl' );
            $grid       = file_get_contents(  'application/views/sximo/module/template/report/index.tpl' ); 
            $model      = file_get_contents(  'application/views/sximo/module/template/report/model.tpl' ); 
        ...
        if($setting['gridtype'] =='ajax')
        {
            ...
            {
               $view = file_get_contents( 'application/views/sximo/module/template/ajax/view_detail.tpl' );
            } else {
          ...
          //  $class = ucwords($class);
            if($this->input->get('rebuild') =='')
            {
                ...
                {               
                    ...
                    file_put_contents(  "application/controllers/{$ctr}.php" , $build_controller) ;    
                    file_put_contents(  "application/models/{$ctr}model.php" , $build_model) ;
                    file_put_contents(  "application/views/{$class}/index.php" , $build_grid) ;       
        ...
        /*
            ...
            if($row->module_db_key =='')
            {
              ...
              if(isset($config['subgrid']) && count($config['subgrid'])>=1)
              {
                   $view = file_get_contents( 'application/views/sximo/module/template/view_detail.tpl' );
              } else {
        ...
        */
        if($this->input->is_ajax_request())
        {
            ...
            echo json_encode(array(
                'status'    =>'success',
                'message'   => 'Code Script has been replaced successfull'
                )); 
     ...
     function savesubform()
    {
        ...
        {   
            ...
            $this->session->set_flashdata('message',SiteHelpers::alert('success','Module Forms Has Been Saved Successfull'));        
            redirect('sximo/module/subform/'.$row->module_name,301);  

        }    else {

            $this->session->set_flashdata('message',SiteHelpers::alert('error','The following errors occurred'));        
            redirect('sximo/module/subform/'.  $this->uri->segment(4),301);
    ...
    function subform( $id ='')
    {
        ...
        $config = SiteHelpers::CF_decode_json($row->module_config); 
        //   echo '<pre>'; print_r($config['subform']); echo '</pre>';    exit;
        $this->data['row'] = $row;
        $this->data['fields'] = $config['grid'];
        ...
        $this->data['content'] = $this->load->view('sximo/module/subform',$this->data, true );        
        $this->load->view('layouts/main', $this->data ); 
   

    } 

    function subformremove( $id =0)
    {
      ...
      SiteHelpers::alert('success','Module Forms Has Been Removed Successfull');
      redirect('sximo/module/subform/'.$row->module_name,301);  
    }            
