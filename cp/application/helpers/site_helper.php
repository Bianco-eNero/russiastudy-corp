...
class SiteHelpers
{
	...
	public static function showUploadedFile($file,$path , $width = 50) {
		...
		//	echo $files ;
			...
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" || $info['extension'] == "gif") 
			{
				...
				<img src="'.base_url(''). $path_file . $file.'" border="0" width="'. $width .'" class="img-circle" /></a></p>';
			} else {
				$path_file = base_url("./","",$path);
				return '<p> <a href="'.base_url($path_file . $file).'" target="_blank"> '.$file.' </a>';
			}
	...
	{
		$_this = & get_Instance();
		$status = $_this->session->flashdata('msgstatus');
		if( $_this->session->flashdata('msgstatus') !=''): ?>	  
		<script type="text/javascript">
            $(document).ready(function(){
			toastr.<?php echo $status;?>("<?php echo $_this->session->flashdata('msgstatus');?>", "<?php echo $_this->session->flashdata('messagetext');?>");
			toastr.options = {
				  "closeButton": true,
				  "debug": false,
	...
	public static function alert_old( $task , $message)
	{
		if($task =='error') {
	...
	public static function avatar( $width =75)
	{
		...
		if($row->avatar !='' ) 	
		{
			...
			{
				return  '<img src="'.base_url().'uploads/users'.'/'.$row->avatar.'" border="0" width="'.$width.'" class="img-circle" />';
			} else {
				return $avatar;
			}	
		} else {
			...
			foreach($forms as $form)
			{
				...
				if($form['view'] != 0)
				{
					if($form['field'] !='entry_by')
					{
						...
						$label = "<?php echo SiteHelpers::activeLang('".$form['label']."',(isset(\$fields['".$form['field']."']['language'])? \$fields['".$form['field']."']['language'] : array()))  ?>";
							...
							{			
								$f .= '					
								  <div class="form-group '.$hidethis.' '.$inhide.'" '.$show .'>
									<label for="'.$form['label'].'" class=" control-label col-md-4 text-left"> '.$label.' '.$required.'</label>
									<div class="col-md-8">
								...
								$f .= '					
								  <div class="form-group '.$hidethis.' '.$inhide.'" '.$show .'>
									<label for="ipt" class=" control-label "> '.$label.'  '.$required.' '.$tooltip.' </label>									
									  '.self::formShow($form['type'],$form['field'],$form['required'],$form['option']).' 						
								  </div> '; 							
	...
	public static function toMasterDetail( $info )
	{
		 ...
		 if(count($info)>=1)
		 {
		 	...
		 	$data['masterdetailsave'] 	= "\$this->detailviewsave( '".$info['module']."', \$this->info['config']['subform'] , \$ID) ;";
		 	$data['masterdetailsubform']= "\$this->data['subform'] = \$this->detailview('".$info['module']."' ,  \$this->info['config']['subform'] ,\$id );";
		 	$tpl = array();
		 	require_once('application/views/sximo/module/template/masterdetailform.php');
	...
	public static function toView( $grids )
	{
		...
		foreach($grids as $grid)
		{
			...
			if($conn['valid'] =='1') {
				$c = implode(':',$conn);
				$val = "<?php echo SiteHelpers::formatLookUp(\$row['".$grid['field']."'],'".$grid['field']."','$c') ;?>";
			}
			...
			if($grid['detail'] =='1')  
			{
				...
				if(isset($match[1]))
				{
					$real_value = "<?php echo \$row['".$match[1]."'] ;?>";
				...
				if($format_as =='radio' or $format_as =='file' or $format_as == 'checkbox' or $format_as =='image'){
					$val = "<?php echo SiteHelpers::formatRows(\$row['".$grid['field']."'],\$fields['".$grid['field']."'],\$row ) ;?>";
					...
					$val = "<?php echo date('".$format_value."',strtotime(\$row['".$grid['field']."'])) ;?>";
				...
				$label = "<?php echo SiteHelpers::activeLang('".$grid['label']."',(isset(\$fields['".$grid['field']."']['language'])? \$fields['".$grid['field']."']['language'] : array()))  ?>";
				$f .= "
					<tr>
	...
	public static  function transForm( $field, $forms = array(),$bulk=false , $value ='')
	{
		...
		switch($type)
		{
			...
			case 'text';			
				$form = "<input  type='text' name='".$field."{$bulk}' class='form-control input-xs' $mandatory value='{$value}'/>";
				break;

			case 'text_date';
				$form = "<input  type='text' name='$field{$bulk}' class='date form-control input-xs' $mandatory value='{$value}'/> ";
				break;
	...
	public static function formatLookUp($val , $field, $arr )
	{
		...
		if(isset($arr['0']) && $arr['0'] ==1)
		{
			...
			//echo " SELECT ".str_replace("|",",",$arr['3'])." FROM ".$arr['1']." WHERE ".$arr['2']." = '".$val."' ";
			if(count($row) >= 1 )
			{
			...
			$values = explode(',',$value);
				...
				foreach($values as $val)
				{
					if($val != '')
					{
						...
						$vals .= '<a href="'.base_url( $format_value . $val).'" target="_blank" class="previewImage"><img src="'.base_url( $format_value . $val ).'" border="0" width="30" class="img-circle" style="margin-right:2px;" /></a>';
					}
				}	 
		    $value = $vals;
	...
	{
		$_this = & get_Instance();
		$activeLang = $_this->session->userdata('lang');
